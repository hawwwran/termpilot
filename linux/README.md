...
[`../ARCHITECTURE.md`](../ARCHITECTURE.md). This file is the Linux/macOS setup guide.
...
1. Upload everything under `relay/` to a web-accessible directory on
   the host (e.g. `public_html/term/`). The bundled `.htaccess` blocks
   `data/`, `logs/`, `config.php`, and common editor backups
   (`*.bak`, `*.swp`, `*.dist`, hidden dotfiles).
   The `relay/lib/vendor/` tree (xterm.js + jsQR, MIT / Apache-2.0; see
   `relay/lib/vendor/NOTICE`) is uploaded verbatim — no CDN is hit at
...
The bundled `tools/deploy.sh` ships `relay/` over FTPS using `~/.netrc`
...
Or, if you've already downloaded `termpilot-linux-macos.zip` from a GitHub
release and extracted it somewhere, just run the bundled installer:
...
Run the Python test suite (`linux/tests/run-all.sh`) before deploying.

## Layout (end-user zip — flat)

What you see after extracting `termpilot-linux-macos.zip`:
...
shared/                      cross-platform Python modules (crypto.py, release_channel.py)
```

The Windows port ships as a separate `termpilot-windows.zip`; see the
`windows/` directory in the source tree.

## Layout (dev tree — nested)
...
│   └── README.md                this file
├── windows/                     Windows wrapper + installers + lib
├── shared/                      crypto.py + release_channel.py (used by both wrappers)
