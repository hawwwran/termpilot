"""
Release-channel awareness for TermPilot — shared between Linux/macOS
and Windows wrappers.

Implements the `--version` and `--update` subcommands. Lives in its own
module so the main wrappers stay focused on the PTY/relay bridge.
...
from a dev checkout still resolves and runs the platform installer
(install-latest-version.sh on Linux/macOS, install-latest-version.bat
on Windows), shadowing the dev tree until the user re-runs the dev
install.

Platform detection is intentionally minimal: this module does not care
which OS it's on, except where the storage location of the pending
update file differs. The installer to invoke is whichever of
install-latest-version.{sh,bat} happens to sit next to the wrapper.
"""
...
def _config_dir() -> Path:
    """Platform-appropriate per-user config dir. Linux/macOS use
    ~/.config/termpilot/; Windows uses %APPDATA%\\termpilot\\."""
    if sys.platform == "win32":
        base = os.environ.get("APPDATA")
        if base:
            return Path(base) / "termpilot"
        return Path.home() / "AppData" / "Roaming" / "termpilot"
    return Path.home() / ".config" / "termpilot"
...
def _confirm(prompt: str) -> bool:
    """Read y/n from stdin (default yes)."""
...
def _run_installer(script_dir: str) -> int:
    ...
    if os.path.isfile(bat):
        return subprocess.call(["cmd.exe", "/c", bat])
    sys.stdout.write(
        "No install-latest-version script next to this wrapper.\n"
        f"Download the latest release from {GITHUB_RELEASES}\n"
    )
    return 1
...
def peek_pending_update_notice(script_dir: str):
    ...
    if a notice should be shown — without writing anything. Also re-saves
...
def spawn_async_update_check(script_dir: str) -> None:
    """Fire-and-forget background thread. Writes PENDING_NOTICE_FILE if
    a newer release exists, removes it if caught up, no-ops if GitHub
    is unreachable. Skips in dev checkouts. Daemon → dies with the
    wrapper."""
    if _is_dev_tree(script_dir):
        return
